...
def main():
    # Header
    st.markdown('<p class="main-header">🏠 Boston Housing Price Predictor</p>',
                unsafe_allow_html=True)
    ...
    with st.expander("🔢 Batch Predictions", expanded=False):
        ...
        st.markdown(
            "**Required columns:** CRIM, ZN, INDUS, CHAS, NOX, RM, AGE, DIS, RAD, TAX, PTRATIO, B, LSTAT")
                    ...
                    with st.spinner(f"Processing {len(df)} predictions..."):
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                df['Predicted_Price_k'] = [r['predicted_price']
                                                           for r in results]
                                df['Predicted_Price_Dollars'] = [
                                    r['predicted_price_dollars'] for r in results]

                                st.success(
                                    f"✅ Predicted prices for {len(df)} houses")
                                st.dataframe(df)
                                ...
                                with col1:
                                    st.metric(
                                        "Average Price", f"${df['Predicted_Price_Dollars'].mean():,.0f}")
                                with col2:
                                    st.metric(
                                        "Min Price", f"${df['Predicted_Price_Dollars'].min():,.0f}")
                                with col3:
                                    st.metric(
                                        "Max Price", f"${df['Predicted_Price_Dollars'].max():,.0f}")
